...
# learning
parser.add_argument('-modeltype', type=str, default='Simple XGBoost', help='model type')
...
parser.add_argument('-epochs', type=int, default=1, help='number of epochs for train [default: 5]')
...
parser.add_argument('-identifier', type=str, default='', help='cv or full run')
...
args.save_dir = os.path.join(
        args.save_dir, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '_' + args.identifier)
args.plot_dir = args.save_dir
#os.path.join(args.plot_dir, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
if not os.path.isdir(args.save_dir): os.makedirs(args.save_dir)
args.filepath = os.path.join(args.save_dir, 
                             args.modeltype + '_' + str(args.static) + '.txt')
args.threshold = float(1)/(1 + args.imbalance_factor)
